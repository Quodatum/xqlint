(:~ 
 : This <a href="https://web.archive.org/web/20220623230943/https://docs.basex.org/web/20220623231027/https://docs.basex.org/wiki/Module_Library">XQuery Module</a> contains functions for accessing specific WebSocket functions. This module is mainly useful in the context of <a href="https://web.archive.org/web/20220623230943/https://docs.basex.org/web/20220623231027/https://docs.basex.org/wiki/WebSockets">WebSockets</a>.
 :
 : @author BaseX Team
 : @see https://web.archive.org/web/20220623231014/https://docs.basex.org/wiki/WebSocket_Module
 :)
...
(:~ 
 : Broadcasts a <code>$message</code> to all connected clients except to the caller. Invocations of this convenience function are equivalent to <code>ws:send($message, ws:ids()[. != ws:id()])</code>. See <a href="https://web.archive.org/web/20220623231014/https://docs.basex.org/wiki/WebSocket_Module#ws:send">ws:send</a> for more details on the message handling.
 :
...
(:~ 
 : Emits a <code>$message</code> to all connected clients. Invocations of this function are equivalent to <code>ws:send($message, ws:ids())</code>. See <a href="https://web.archive.org/web/20220623231014/https://docs.basex.org/wiki/WebSocket_Module#ws:send">ws:send</a> for more details on the message handling.
 :
 : @param $message value of type xs:anyAtomicType
 :)
...
(:~ 
 : Schedules the evaluation of the supplied <code>$query</code> and returns the result to the calling WebSocket client. The query can be a URI or a string, and variables and context items can be declared via <code>$bindings</code> (see <a href="https://web.archive.org/web/20220623230943/https://docs.basex.org/web/20220623231027/https://docs.basex.org/wiki/XQuery_Module#xquery:eval">xquery:eval</a> for more details). The following <code>$options</code> can be supplied: <ul> <li> <code>base-uri</code>: sets the <a href="https://web.archive.org/web/20220623231027/https://www.w3.org/TR/xquery-31/#dt-static-base-uri">base-uri property</a> for the query. This URI will be used when resolving relative URIs, such as with <code>fn:doc</code>.</li> <li> <code>id</code>: sets a custom job id. The id must not start with the standard <code>job</code> prefix, and it can only be assigned if no job with the same name exists.</li> </ul> <p>Query scheduling is recommendable if the immediate query execution might be too time consuming and lead to a timeout. </p>
 :
 ...
 :)
declare function ws:eval($query as xs:anyAtomicItem) as xs:string external;
...
(:~ 
 ...
 : @error ws:id The specified id is invalid or has already been assigned.
 :)
declare function ws:eval($query as xs:anyAtomicItem, $bindings as map(*)?) as xs:string external;
...
(:~ 
 ...
 : @error ws:id The specified id is invalid or has already been assigned.
 :)
declare function ws:eval($query as xs:anyAtomicItem, $bindings as map(*)?, $options as map(*)?) as xs:string external;

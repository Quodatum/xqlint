...
var completeType = function (identifier, pos, sctx) {
    var matches = findCompletions(identifier, XQUERY_TYPES);
...
var completeExpr = function (line, pos, sctx) {
    var identifier = retrievePrecedingIdentifier(line, pos.character, nameCharRegExp);
    //console.log("COMPLETER: ",identifier,pos,line[pos.character-1])
    var before = line.substring(0, pos.character - (identifier.length === 0 ? 0 : identifier.length));
    ...
    if (isVar) {
        ...
        return completeFunction(identifier, pos, sctx)
                .concat(completePrefix(identifier, pos, sctx));
    } else {
        return completeVariable(identifier, pos, sctx)
                .concat(completeFunction(identifier, pos, sctx))
                .concat(completePrefix(identifier, pos, sctx));
    }
};

var completeModuleUri = function (line, pos, sctx) {
    var identifier = retrievePrecedingIdentifier(line, pos.character, uriRegex);
...
exports.complete = function (source, ast, rootSctx, pos) {
    ...
    if(!node){
        node = TreeOps.findNode(ast, pos,true); 
    }
    var sctx = TreeOps.findNode(rootSctx, pos);
    sctx = sctx ? sctx : rootSctx;
    if(node && node.value === 'as' || TreeOps.ancestor(node,"TypeDeclaration")){
        var identifier = retrievePrecedingIdentifier(line, pos.character, nameCharRegExp);
        return completeType(identifier, pos, sctx);
    } 
    node = (node && node.name === 'StringLiteral') ? node.getParent : null;

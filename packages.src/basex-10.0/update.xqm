...
(:~ 
 : The updating variant of <a href="https://docs.basex.org/wiki/Higher-Order_Functions#fn:for-each">fn:for-each</a> applies the specified updating <code>$function</code> to every item of <code>$seq</code>.
 :
...
(:~ 
 : The updating variant of <a href="https://docs.basex.org/wiki/Higher-Order_Functions#fn:for-each-pair">fn:for-each-pair</a> applies the specified updating <code>$function</code> to the successive pairs of items of <code>$seq1</code> and <code>$seq2</code>. Evaluation is stopped if one sequence yields no more items.
 :
...
(:~ 
 : The updating variant of <code> <a href="https://docs.basex.org/wiki/Map_Module#map:for-each">map:for-each</a> </code> applies the specified <code>$function</code> to every key/value pair of the supplied <code>$map</code> and returns the results as a sequence.
 :
...
(:~ 
 : Returns the items that have been cached by <code> <a href="https://docs.basex.org/wiki/Update_Module#update:output">update:output</a> </code>. The output cache can optionally be <code>$reset</code>. The function can be used to check which items will eventually be returned as result of an updating function.<br/>This function is <i>non-deterministic</i>: It will return different results before and after items have been cached. It is e. g. useful when writing <a href="https://docs.basex.org/wiki/Unit_Module">unit tests</a>.
 :
 : @return value of type item()*
 :)

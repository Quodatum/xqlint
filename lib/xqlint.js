...
function XQLint(source, opts) {
        ...
        for (const [key, value] of Object.entries(ns)) {
            if (value.ats) {
                ...
                if (a1.baseUri) {
                    ...
                    var dlink = { ns: key, path: location, pos: a1.pos };
                    doclinks.push(dlink);
                }
            }
        }
    ...
    this.getMarkers = function (level) {
        var m = [];
        markers.forEach(function (marker) {
            if (marker.level === level || level === undefined) {
                m.push(marker);
            }
        });
        return m;
    ...
    try {
        ...
        if (e instanceof parser.ParseException) {
            ...
            // unparsed extent if text after error
            if (source.length - e.getEnd() > 2) {
                pos = convertPosition(source, e.getEnd() + 1, source.length - 1);
                markers.push({
                    pos: pos,
                    code: 'XQLT0001',
                    level: 'error',
                    message: "[XQLT0001] " + "Unparsed due to previous parser error."
                });
            }
        } else {
            throw e;
...
const cacheMods = {}; //map target->xqdoc 
...
function resolver1(uri, ats) {//uri, hints
    ...
    if (!cacheMods[target]) {
        const opts={"fileName": target,processor: this.processor};
        const linter = new XQLint(fs.readFileSync(target, 'utf-8'),opts);
        cacheMods[target] = linter.getXQDoc(true);
    }
    return cacheMods[target];
}

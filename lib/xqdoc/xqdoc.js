...
exports.XQDoc = function (ast, sctx) {
    ...
    this.VarDecl = function (node) {
        ...
        lintRanges.push({ key: key, type: "VarDecl", pos: node.getParent.pos });
    ...
    this.FunctionDecl = function (node) {
        ...
        lintRanges.push({ key: key, type: "FunctionDecl", pos: node.getParent.pos }); // save range of enclosing declaration
    ...
    this.getXQDoc = function (withPos) {
        ...
        lintRanges.filter(v => v.type === 'VarDecl').forEach((v) => {
            const key = v.key;
        ...
        lintRanges.filter(v => v.type === 'FunctionDecl').forEach((v) => {
            const key = v.key;

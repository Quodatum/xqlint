...
declare variable $qipe:outputs := map { "html5" : map { "method" : "html",
    "version" : "5.0" }, "xml" : map { "indent" : "no" } };
...
declare function qipe:step($acc, $this as element(*), $opts as map (*)) {
    ...
    case element(qipe:validate)
        return qipe:validate($acc, $this, $opts)
    case element(qipe:xquery)
        return qipe:xquery($acc, $this, $opts)
    case element(qipe:xslt)
        return qipe:xslt($acc, $this, $opts)
    case element(qipe:load)
        return qipe:load($acc, $this, $opts)
    case element(qipe:store)
        return qipe:store($acc, $this, $opts)
    case element(qipe:directory-list)
        return qipe:dir($acc, $this, $opts)
    case element(qipe:pipeline)
        return qipe:pipeline($acc, $this, $opts)
    case element(qipe:identity)
        return qipe:identity($acc, $this, $opts)
    default
        return error(xs:QName('qipe'), 'unknown step:' || name($this)) };

(:~  subpipe
 :)
declare function qipe:pipeline($acc, $this as element(qipe:pipeline),
                               $opts as map (*)) { let $a :=
    qipe:run($this, $acc)
return $acc };
...
(:~  identity
 :)
declare function qipe:identity($acc, $this as element(qipe:pipeline),
                               $opts as map (*)) { $acc => trace("IDENT") };

(:~  run validate step based on @type
 :)
declare function qipe:validate($acc, $this as element(qipe:validate),
                               $opts as map (*)) { let $href :=
    qipe:resolve($this/@href)
let $failOnError := boolean($this/@failOnError)
let $fn :=
    switch ($this/@type/string())
        case "relax-ng"
            return qipe:relax-ng(?, $href)
        case "schematron"
            return qipe:schematron(?, $href)
        case "xml-schema"
            return qipe:validate-xsd(?, $href)
        case "dtd"
            return qipe:validate-dtd(?, $href)
        default
            return error(xs:QName('qipe'),
                         'unknown validation type: ' || $this/@type/string())
for $doc at $i in $acc
let $report := $fn($doc)
...
(:~
 ...
 :)
declare function qipe:dir($acc, $this as element(qipe:directory-list),
                          $opts as map (*)) { 
let $inc := $this/@include-filter/string()
let $href := $this/@href/string()
let $result := fw:directory-list($href, map { "include-filter" : $inc })
let $result :=
    document { $result } transform with { delete node//c:directory[not(.//c:file
                                                                      )] }
return ($acc, $result) };
...
declare function qipe:xquery($acc, $this as element(qipe:xquery),
                             $opts as map (*)) { let $href :=
    resolve-uri($this/@href, base-uri($this)) => trace("AT")
...
let $name := xquery:eval($eval, map { "" : $item, "position" : $pos })
(: eval against doc :) let $dest := $href || "/" || $name
let $dir := file:parent($dest)
return
    ($item, if (file:is-dir($dir)) then
        ()
    else
        file:create-dir($dir)
    ,
...
as element(report) { validate:xsd-report($doc, $xsd) };

(:~  validate with dtd :)
declare function qipe:validate-dtd($doc, $dtd)
as element(report) { validate:dtd-report($doc, $dtd) };

(:~  validate with relax-ng :)
declare function qipe:relax-ng($doc, $rng)
as element(report) { let $compact := matches($rng, ".*\.rnc")
...
as element(report) { let $sch := schematron:compile(doc($uri-sch))
let $svrl := schematron:validate($doc, $sch)
return
         <report>
            <status>{
    if (schematron:is-valid($svrl)) then
        'valid'
    else
        'invalid'
            }</status>
...
declare function qipe:load($acc, $this, $opts as map (*)) { let $href :=
    qipe:resolve($this/@href) => trace("load")
let $new :=
    if (file:is-dir($href)) then
        error(xs:QName('qipe'), 'dir loading not implemented')
    (: @TODO load all :) else
        doc($href)
return ($acc, $new) };

(:~  resolve locations relative to this document typically from @href :)
declare function qipe:resolve($href as node()?) { resolve-uri($href,
                                                              base-uri($href))
    };

(:~  log msg :)
declare function qipe:log($text as xs:string, $opts as map (*))
as empty-sequence () { if ($opts? log) then

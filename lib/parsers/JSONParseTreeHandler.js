...
exports.JSONParseTreeHandler = function (code) {
    ...
    var list = [
        'OrExpr', 'AndExpr', 'ComparisonExpr', 'FTContainsExpr', 'StringConcatExpr', 'RangeExpr',
        'AdditiveExpr', 'MultiplicativeExpr',
        'UnionExpr', 'IntersectExceptExpr', 'InstanceofExpr', 'TreatExpr', 'CastableExpr', 'CastExpr',
        'TransformWithExpr', 'BasexUpdate1Expr', 'BasexUpdateExpr', 'ArrowExpr',
        'UnaryExpr', 'ValueExpr',
        'SimpleMapExpr', 'PathExpr', 'RelativePathExpr', 'StepExpr', 'PostfixExpr', 'PrimaryExpr'
    ];
    ...
    function setValue(node, begin, end) {
        ...
        var e = end - cursor;
        //console.log("set: ",node.name,"=", remains.substring(0, e))
    ...
    function popNode() {
        ...
        if (ptr.name === 'FunctionName') {
         ...
         if (ptr.name === 'EQName' && ptr.value === undefined && ptr.children.length >0) {
            //console.log("::::::",ptr.children[0].name)

            ptr.value=ptr.children[0].value;
            if(ptr.value === undefined){ 
                ptr.value=ptr.children[0].children[0].value   
              //console.log("AA:",ptr.value )
            }
            ptr.children.pop()
         }
    ...
    this.endNonterminal = function (name, end){
        popNode();
         //console.log("endNonterminal",name,end)
    };
    ...
    this.terminal = function (name, begin, end) {
        ...
        //console.log("TERMINAL:",name,ptr.value)
        popNode();
    };

    this.whitespace = function (begin, end) {
        //console.log("WS:",begin,end)
        var name = 'WS';
        pushNode(name, begin);
        setValue(ptr, begin, end);
        popNode();
